    ...
    public class LoginController : Controller
    {
			...
			if (isPost)
			{
				...
				using (MySqlConnection con = new MySqlConnection(constr))
		        {
			        try
			        {
						...
						if (PasswordStorage.VerifyPassword(Request["loginPassword"],$"sha1:64000:18:{login.Salt}:{login.Hash}"))
							login.LoggedIn = true;
	    ...
	    public ActionResult Register()
	    {
		    ...
		    if (isPost)
		    {
			    ...
			    ModelState.AddModelError("Hash", result);

			    if (!PasswordStorage.VerifyPassword(Request["password"], result))
					ModelState.AddModelError("Error", "no u");
